...
export function combineItems(first: CoverageItem, second: CoverageItem): CoverageItem {
  const percentage = (second.covered + first.covered) > 0 ?
    100 * (first.covered + second.covered) / (second.total + first.total) : 100
  return {
...
export function parseCoverageModel(coveragePath: string): CoverageModel | undefined {
  ...
  if (!filesystem.exists(resolvedPath)) {
    throw Error(`Couldn't find instanbul coverage json file at path '${resolvedPath}'.`)
  }
  ...
  try {
    ...
    throw Error(`Coverage data had invalid formatting at path '${resolvedPath}'`)
  }
}
